...
class AgentLoaded(AgentMessage, namespace="instrukt"):
    agent: NeuronCLIAgent

    def __init__(self, agent: NeuronCLIAgent) -> None:
        if not isinstance(agent, NeuronCLIAgent):
            raise ValueError(f"agent must be an NeuronCLIAgent, got {agent}")
        self.agent = agent
        super().__init__(AgentEvents.AgentLoad)

    @property
    def value(self) -> t.Optional[NeuronCLIAgent]:
        return self.agent
