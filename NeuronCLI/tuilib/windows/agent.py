...
from ...types import NeuronCLIDomNodeMixin
...
if t.TYPE_CHECKING:
    from instrukt.agent.base import NeuronCLIAgent
...
class AgentStatusBar(Horizontal, NeuronCLIDomNodeMixin):
...
class AgentWindow(Container, NeuronCLIDomNodeMixin):
...
class AgentConversation(VerticalScroll, NeuronCLIDomNodeMixin, can_focus=False):
    """Displays the conversation with the active agent."""

    _chat_messages: reactive[list["ChatMessage"]] = reactive([])

    @work(exclusive=True)
    async def preload_messages(self, agent: "NeuronCLIAgent") -> None:
    ...
    async def handle_agent_loaded_msg(self, message: AgentLoaded):
        ...
        agent: "NeuronCLIAgent" = message.agent
