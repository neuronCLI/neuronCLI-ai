...
class NeuronCLIAgent(BaseModel, ABC):
    """NeuronCLI agents need to satisfy this base class.

    .. code-block::
        :caption: defining a custom agent

        class MyAgent(NeuronCLIAgent):
            name = "my_agent"
    ...
    def __init_subclass__(cls: Type["NeuronCLIAgent"], **kwargs):
    ...
    @classmethod
    @abstractmethod
    def load(cls, ctx: 'Context') -> Optional['NeuronCLIAgent']:
        """Agent loading logic goes here."""

    #TODO!: API for async agent loading
    # @classmethod
    # @abstractmethod
    # async def aload(cls, ctx: 'Context') -> Optional['NeuronCLIAgent']:
    ...
    async def send_message(self, ctx: Context, msg: str) -> None:
        ...
        from .callback import NeuronCLICallbackHandler
        instrukt_cb_handler = NeuronCLICallbackHandler(ctx=ctx)
