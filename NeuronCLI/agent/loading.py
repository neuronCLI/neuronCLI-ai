...
from .base import NeuronCLIAgent
...
class ModuleManager:
    ...
    @staticmethod
    def verify_module(name: str) -> Type[NeuronCLIAgent]:
        ...
        for path in AGENT_MODULES_PATHS:
            ...
            # the main.py file must have a class the implements NeuronCLIAgent class
            mod = importlib.import_module(f"{name}.{MODULE_ENTRY_POINT[:-3]}")
            for _, agent_cls in inspect.getmembers(mod, inspect.isclass):
                if issubclass(agent_cls, NeuronCLIAgent) and agent_cls is not NeuronCLIAgent:
...
class AgentLoader:
    ...
    @staticmethod
    def load_agent(name: str, ctx: Context) -> Optional[NeuronCLIAgent]:
        """Load an agent module by name.

        The agent must be a subdirectory of the agent_modules directory and
        MUST implement the NeuronCLIAgent base class.
        """
